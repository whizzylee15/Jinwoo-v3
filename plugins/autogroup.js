...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!m.isGroup) return m.reply("*❗️ THIS COMMAND CAN ONLY BE USED IN GROUPS❗️*");
    ...
    if (!botAdmin) return m.reply("*❗️ BOT MUST BE AN ADMIN TO USE THIS COMMAND❗️*");
    if (!senderAdmin) return m.reply("*❗️ YOU MUST BE AN ADMIN TO USE THIS COMMAND❗️*");
